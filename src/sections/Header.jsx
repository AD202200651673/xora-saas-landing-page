...
import { Link as LinkScroll } from "react-scroll";   // installed react-scroll libelary
import clsx from "clsx";   // installed clsx libelary for giving conditions in tailwind classes
...
const Header = () => {
  ...
  const NavLink = ({ title }) => (
    <LinkScroll onClick={() => setIsOpen(false)} to={title} offset={-100} spy smooth activeClass="nav-active" className="base-bold text-p4 uppercase transition-colors duration-500 cursor-pointer hover:text-p1 max-lg:my-4 max-lg:h-5">
      {title}
    </LinkScroll>
  );
  ...
  return (
    <header className={clsx('fixed top-0 left-0 z-50 w-full py-10 transition-all duration-500 max-lg:py-4', hasScrolled && 'py-2 bg-black-100 backdrop-blur-[8px]')}>
      <div className="container flex h-14 items-center max-lg:px-5">
        ...
        <img src={isOpen ? close : magic} alt="logo" className="size-1/2 object-contain"/>
        </button>
      </div>
    </header>
  );
